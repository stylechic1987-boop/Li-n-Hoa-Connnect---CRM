...
export default function App(){
 ...
 async function checkOut(e:Emp){
   const a=attendance.find(x=>(x.employee===e.user_id||x.employee===e.id||x.employee===e.db_id)&&x.date===today); if(isLive&&supabase&&a){
 ...
 {tab==="attendance"&&<div className="card"><h3>Chấm công hôm nay</h3><p className="muted">Ca mặc định: 08:00–17:30 • nghỉ 60 phút • dung sai 10 phút {busy&&"• đang đồng bộ…"}</p>{employees.filter(e=>e.status==="Đang làm").map(e=>{const a=checked.find(x=>x.employee===e.user_id||x.employee===e.id);const self=profile?.employee_id===e.id || profile?.user_id===e.user_id;return <div key={e.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 0",borderTop:"1px solid #eee"}}><div><b>{e.name}</b><div className="muted">{roleName[e.role]||e.role} • {e.branch}</div></div><div>{a?.in&&<span className="badge ok" style={{marginRight:8}}>Vào {a.in}</span>}{a?.out&&<span className="badge ok">Ra {a.out}</span>}{!a?.in&&(demo||canManage||self)&&<button className="btn" onClick={()=>checkIn(e)}><LogIn size={14}/> Bắt đầu ca</button>}{a?.in&&!a?.out&&(demo||canManage||self)&&<button className="btn gray" onClick={()=>checkOut(e)} style={{marginLeft:8}}><LogOut size={14}/> Kết thúc ca</button>}</div></div>})}</div>}
