...
export default function App(){
 ...
 return <div className="app"><aside className="side"><div className="brand">LIÊN HOA<small>CONNECT • HR & CRM</small></div><div className="nav">{nav.map(([id,label,I]:any)=><button className={tab===id?"active":""} onClick={()=>setTab(id)} key={id}><I size={16} style={{verticalAlign:"middle",marginRight:9}}/>{label}</button>)}</div><div style={{position:"absolute",bottom:22,color:"#9ca3af",fontSize:11}}>Liên Hoa Global Education</div></aside>
